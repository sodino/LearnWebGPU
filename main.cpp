...
wgpu::RequiredLimits Application::GetRequiredLimits(wgpu::Adapter adapter) const {
    ...
    requiredLimits.limits.maxBufferSize = 7 * 6 * sizeof(float); // 7个顶点，每个顶点一对(x,y,z) + rgb共6个值，每个值都是float。(由于有index优化的作用，实际用不到 7*6 )
...
void Application::InitializeBuffers() {
    ...
    std::vector<float> pointData = {
            // x0, y0, z0,         r0,  g0,  b0
            //                                    // 金字塔底座4个点，z统一为 -0.3
            -0.5, -0.5, -0.3,      1.0, 0.0, 0.0, // 左下 0
            +0.5, -0.5, -0.3,      0.0, 1.0, 0.0, // 右下 1
            +0.5, +0.5, -0.3,      0.0, 0.0, 1.0, // 右上 2
            -0.5, +0.5, -0.3,      1.0, 1.0, 0.0, // 左上 3

            +0.0, +0.0, +0.5,      0.5, 0.5, 0.5  // 金字塔顶点，z为 +0.5
    };
